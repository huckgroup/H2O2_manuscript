...
```{r tidy_Bas013}
# Combine all data into one tidy dataframe (all data in one column instead of one column for each fluor)
cols_meta_Bas013 <- c("expID", "plateID", "wellID", "sample", "stimulus", "conc_final", "stim_profile", "gradient_duration", "stim_description", "stim_descrip_short", "time_min", "time_point", "after_end", "conc_at_time_mM", "cumulative_exposure", "staining", "panel", "sample_cell_count") # include all metadata columns
...
Save annotated datasets as .csv file
```{r save_DS096}
write.csv(df_tidy_DS096, file = "output/annotated_data_tidy_DS096.csv", row.names = F)

write.csv(medians_tidy_DS096, file = "output/annotated_medians_tidy_DS096.csv", row.names = F)
...
Save annotated datasets as .csv file
```{r save_DS097}
write.csv(df_tidy_DS097, file = "output/annotated_data_tidy_DS097.csv", row.names = F)

write.csv(medians_tidy_DS097, file = "output/annotated_medians_tidy_DS097.csv", row.names = F)
...
Load the gated dataset and create a dataset with live single cells
```{r data_DS098}
gs_DS098 <- load_gs(paste0("output/", "gated_data_DS098"))

gated_pops_DS098 <- gs_get_pop_paths(gs_DS098, path = "auto")
cs_DS098 <- gs_pop_get_data(gs_DS098, tail(gated_pops_DS098, 1))

# Remove all zero values from df in order to perform log transformations
df_DS098 <- fortify(cs_DS098)
df_DS098[df_DS098 <= 0] <- NA

# Select only columns that are needed
df_DS098 <- df_DS098[ , c("name", "expID", "rowID", "colID", "wellID", "FSC.A", "SSC.A", "BV421.A", "PE.A", "Alexa.647.A", "Alexa.488.A")]
...
# Change columns into character if necessary
metadata_DS098$colID <- as.character(metadata_DS098$colID)

df_DS098 <- left_join(df_DS098, metadata_DS098)

# Remove samples with <5000 cells (except if the sample is t = 0 min)
counts_DS098 <- df_DS098 %>% count(wellID, name = "sample_cell_count")
df_DS098 <- left_join(df_DS098, counts_DS098)
df_DS098 <- df_DS098[df_DS098$sample_cell_count > 5000, ]

gc()

# kable(df_DS098 %>% head(5)) %>% kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r tidy_DS098}
# Combine all data into one tidy dataframe (all data in one column instead of one column for each fluor)
cols_meta_DS098 <- c("expID", "plateID", "wellID", "sample", "stimulus", "H2O2_conc_mM", "time_min", "time_point", "cumulative_exposure", "inhibitor", "replicate", "stim_description", "staining", "panel", "sample_cell_count") # include all metadata columns

df_PE_DS098 <- df_DS098 %>% 
  select(all_of(cols_meta_DS098), protein_PE, PE.A, FSC.A, SSC.A) %>% 
  mutate(fluor = "PE") %>%
  rename(
    protein = protein_PE, 
    fluorescence = PE.A, 
    FSC = FSC.A, 
    SSC = SSC.A)
df_AF647_DS098 <- df_DS098 %>% 
  select(all_of(cols_meta_DS098), protein_AF647, Alexa.647.A, FSC.A, SSC.A) %>% 
  mutate(fluor = "AF647") %>%
  rename(
    protein = protein_AF647, 
    fluorescence = Alexa.647.A, 
    FSC = FSC.A, 
    SSC = SSC.A)
df_AF488_DS098 <- df_DS098 %>% 
  select(all_of(cols_meta_DS098), protein_AF488, Alexa.488.A, FSC.A, SSC.A) %>% 
  mutate(fluor = "AF488") %>%
  rename(
    protein = protein_AF488, 
    fluorescence = Alexa.488.A, 
    FSC = FSC.A, 
    SSC = SSC.A)
df_BV421_DS098 <- df_DS098 %>% 
  select(all_of(cols_meta_DS098), protein_BV421, BV421.A, FSC.A, SSC.A) %>% 
  mutate(fluor = "BV421") %>%
  rename(
    protein = protein_BV421, 
    fluorescence = BV421.A, 
...
df_tidy_DS098 <- list(df_PE_DS098, df_AF647_DS098, df_AF488_DS098, df_BV421_DS098) %>% reduce(full_join)

gc()

# kable(df_tidy_DS098 %>% head(5)) %>% kable_styling(bootstrap_options = c("striped", "hover"))
```

```{r percent_on_DS098}
# Get the value for each conc where in sample t = 0 2.5% of cells are higher
data_info_DS098 <- df_tidy_DS098 %>%
  filter(H2O2_conc_mM == 1 & inhibitor == "DMSO") %>%
  group_by(replicate, protein, fluor) %>%
  summarise(zero_quant97.5 = quantile(fluorescence, c(0.975), na.rm = T))

df_tidy_DS098 <- left_join(df_tidy_DS098, data_info_DS098)

# Create dataframe with percentage ON for each condition
data_total_DS098 <- df_tidy_DS098 %>%
  group_by(wellID, protein, fluor) %>%
  count(name = "count_total")

data_on_DS098 <- df_tidy_DS098 %>%
  filter((fluorescence - zero_quant97.5) > 0) %>%
  group_by(wellID, protein, fluor) %>%
  count(name = "count_on")

data_counts_DS098 <- left_join(data_total_DS098, data_on_DS098) %>%
  mutate(percent_on = (count_on / count_total) * 100)

df_tidy_DS098 <- left_join(df_tidy_DS098, data_counts_DS098)
...
Calculate median fluorescent values for each sample/replicate
```{r medians_sample_DS098}
# Calculate median values for each sample
medians_DS098 <- df_tidy_DS098 %>% 
  group_by(wellID, protein, fluor) %>% 
  summarise_at(vars(fluorescence), list(fluorescence_median = median), na.rm = TRUE)

medians_tidy_DS098 <- left_join(medians_DS098, distinct(select(df_tidy_DS098, cols_meta_DS098))) %>%
  left_join(data_counts_DS098)

kable(medians_tidy_DS098 %>% head(5)) %>% kable_styling(bootstrap_options = c("striped", "hover"))
...
```{r medians_rep_DS098}
# Make a dataframe with means and standard deviation of replicates
medians_avg_DS098 <- medians_tidy_DS098 %>%
  dplyr::group_by(H2O2_conc_mM, inhibitor, protein) %>%
  summarise(
    fluorescence_sd = sd(fluorescence_median, na.rm = T),
    fluorescence = mean(fluorescence_median, na.rm = T), 
    percent_sd = sd(percent_on, na.rm = T),
...
medians_avg_DS098 <- left_join(medians_avg_DS098, select(medians_tidy_DS098, c(expID, stimulus, H2O2_conc_mM, time_min, time_point, cumulative_exposure, protein)), multiple = "all") %>%
  select(-c("wellID")) %>%
  distinct() %>% 
  mutate(stim_description = paste(H2O2_conc_mM, "mM -", inhibitor))

gc()

# kable(medians_avg_DS098 %>% head(5)) %>% kable_styling(bootstrap_options = c("striped", "hover"))
```
...
Save annotated datasets as .csv file
```{r save_DS098}
write.csv(df_tidy_DS098, file = "output/annotated_data_tidy_DS098.csv", row.names = F)

write.csv(medians_tidy_DS098, file = "output/annotated_medians_tidy_DS098.csv", row.names = F)

write.csv(medians_avg_DS098, file = "output/annotated_medians_avg_rep_DS098.csv", row.names = F)

# Remove unnecessary files to clear up memory
rm(list = ls(pattern = "_DS098"))
gc()
```
