...
```{r fig_debris_DS085, fig.width=9, fig.height=6}
...
```{r fig_singlets_DS085, fig.width=9, fig.height=6}
# Apply the gate to all data
...
```{r fig_live_DS085, fig.width=9, fig.height=6}
# Apply the gate to all data
...
```{r fig_live_percent_DS085, fig.width=6, fig.height=3}
...
# Manually adjust the numbers and check in the graph to set right gate
g_live_Bas013 <- polygonGate(filterId = "Live", "FSC.A" = c(4.5e4, 1.6e5, 1.6e5, 4.5e4), "BV421.A" = c(5e2, 5e2, 1e4, 5e3)) # define gate

ggcyto(gs_Bas013[[1]], aes(x = FSC.A, y = BV421.A), subset = "Singlets") +
  geom_hex(bins = 200) +
...
```{r fig_live_percent_Bas013, fig.width=4, fig.height=3}
ggplot(subset(percent_live_Bas013, staining == "yes"), aes(x = time_min, y = PercentRoot)) +
  ...
  scale_color_manual(
    ...
    breaks = unique(percent_live_Bas013$stim_description),
    name = "",
  ) +
  labs(y = "Percentage live single \ncells from root (%)", x = "Stimulation time (min)") +
  theme_bw() +
